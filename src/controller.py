    ...
    def get_USB_status(self):
        ...
        for d in os.listdir(basedir):
            if 'usb' in d and 'part' not in d:
                ...
                temp_found = True
                USB_name = link.split('/')[-1] + '1'
                self.mount_USB(USB_name)
...
class screen:
    ...
    def draw_menu8_screen(self,unmount=False):
        ...
        if self.controller.is_USB_mounted():
            menu8_draw.text((self.line_start,self.line_list[0]),'USB Ejected',font=self.fnt,fill=self.color_white)
            menu8_draw.text((self.line_start,self.line_list[2]),'REMOVE USB',font=self.fnt,fill=self.color_white)
